...
export default function CreateStudyGroupModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError('Failed to create the study group. Please try again.')
      setSubmitting(false)
    }
  }

  return <div role="dialog" aria-modal="true" aria-labelledby="create-group-title" className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(4px)' }} onClick={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <div className="rounded-2xl overflow-hidden w-full" style={{ maxWidth: 520, background: '#FFFFFF', boxShadow: '0 24px 60px rgba(15,23,42,0.22)' }}>
      <div className="px-6 py-5 flex items-start justify-between" style={{ background: '#EEF2FF', borderBottom: '1px solid #E0E7FF' }}><div><h2 id="create-group-title" style={{ fontSize: 19, fontWeight: 800, color: '#0F172A' }}>Create Study Group</h2><p style={{ fontSize: 11, color: '#64748B', marginTop: 3 }}>Start a new study group and invite classmates.</p></div><button onClick={onClose} aria-label="Close" className="rounded-lg p-1.5" style={{ color: '#64748B' }}><X size={18} /></button></div>
      <div className="p-6 space-y-4">
        <div>
          <label htmlFor="sg-course" style={{ fontSize: 11, fontWeight: 700, color: '#334155', marginBottom: 5, display: 'block' }}>Course</label>
          <div className="relative">
            <div className="flex items-center gap-2 rounded-xl px-3 py-2" style={inputStyle}>
              <Search size={13} color="#94A3B8" />
              <input id="sg-course" value={query} onFocus={() => setCourseOpen(true)} onChange={(e) => { setQuery(e.target.value); setCourseOpen(true) }} placeholder="Search for a course..." className="flex-1 min-w-0 bg-transparent outline-none" style={{ fontSize: 12, color: '#1E293B' }} />
            </div>
          ...
          {selected && <div className="mt-2 flex items-center justify-between rounded-xl px-3 py-2" style={{ background: '#EEF2FF', border: '1px solid #C7D2FE' }}><span style={{ fontSize: 11, fontWeight: 700, color: '#4338CA' }}>{selected.code} – {selected.title}</span><button aria-label="Clear course" onClick={() => { setSelectedCourse(null); setQuery(''); setCourseOpen(false) }} style={{ color: '#64748B' }}><X size={13} /></button></div>}
        </div>
        <div>
          <label htmlFor="sg-name" style={{ fontSize: 11, fontWeight: 700, color: '#334155', marginBottom: 5, display: 'block' }}>Group name</label>
          <input id="sg-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. CMPS 214 Finals Squad" style={inputStyle} />
        </div>
        <div>
          <label htmlFor="sg-bio" style={{ fontSize: 11, fontWeight: 700, color: '#334155', marginBottom: 5, display: 'block' }}>Bio / description <span style={{ color: '#94A3B8', fontWeight: 500 }}>(optional)</span></label>
          <textarea id="sg-bio" value={bio} onChange={(e) => setBio(e.target.value)} rows={3} placeholder="What will this group focus on?" style={{ ...inputStyle, resize: 'none' }} />
        </div>
        <div>
          <label style={{ fontSize: 11, fontWeight: 700, color: '#334155', marginBottom: 5, display: 'block' }}>Meeting day(s) <span style={{ color: '#94A3B8', fontWeight: 500 }}>(optional)</span></label>
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button onClick={submit} disabled={!canSubmit} className="flex items-center gap-1.5 rounded-lg px-4 py-2 font-semibold" style={{ fontSize: 12, background: canSubmit ? '#4338CA' : '#C7D2FE', color: '#FFFFFF' }}>{submitting ? <Loader2 size={13} className="animate-spin" /> : null}{submitting ? 'Creating...' : 'Create Group'}</button>
        </div>
      </div>
    </div>
  </div>
